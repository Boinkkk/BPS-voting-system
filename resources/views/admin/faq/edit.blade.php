...
<div class="mb-6 flex items-center justify-between">
    <div>
        <h1 class="text-2xl font-bold text-bps-text mb-1">Edit FAQ</h1>
        <p class="text-sm text-gray-500">Perbarui pertanyaan dan jawaban FAQ.</p>
    </div>
    ...
    <div class="p-6">
        <form action="{{ route('admin.faq.update', $faq->id) }}" method="POST">
            ...
            <div class="mb-5">
                ...
                <input type="text" name="pertanyaan" id="pertanyaan" class="w-full border-gray-300 rounded-md shadow-sm focus:ring-bps-primary focus:border-bps-primary sm:text-sm @error('pertanyaan') border-red-500 @enderror" value="{{ old('pertanyaan', $faq->pertanyaan) }}" required>
                @error('pertanyaan')
            ...
            <div class="mb-6">
                ...
                <textarea name="jawaban" id="jawaban" rows="5" class="w-full border-gray-300 rounded-md shadow-sm focus:ring-bps-primary focus:border-bps-primary sm:text-sm @error('jawaban') border-red-500 @enderror" required>{{ old('jawaban', $faq->jawaban) }}</textarea>
                @error('jawaban')
                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
            </div>
