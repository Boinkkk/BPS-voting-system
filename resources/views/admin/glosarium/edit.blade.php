...
<div class="mb-6 flex justify-between items-end">
    <div>
        <h1 class="text-2xl font-bold text-[#1D1D1B] mb-1">Edit Istilah Glosarium</h1>
    ...
    <form action="{{ route('admin.glosarium.update', $glosarium->id) }}" method="POST">
        ...
        <div class="p-6 space-y-6">
            ...
            <div>
                ...
                <input type="text" name="istilah" id="istilah" value="{{ old('istilah', $glosarium->istilah) }}" class="mt-1 focus:ring-[#0091DA] focus:border-[#0091DA] block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" required>
                @error('istilah')
            ...
            <div>
                ...
                <textarea name="definisi" id="definisi" rows="5" class="mt-1 focus:ring-[#0091DA] focus:border-[#0091DA] block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" required>{{ old('definisi', $glosarium->definisi) }}</textarea>
                @error('definisi')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
