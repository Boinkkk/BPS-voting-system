...
<div class="mb-6 flex justify-between items-end">
    <div>
        <h1 class="text-2xl font-bold text-bps-text mb-1">Edit Istilah Glosarium</h1>
        <p class="text-sm text-gray-500">Perbarui istilah atau definisi glosarium.</p>
    </div>
    <div>
...
<div class="bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm max-w-3xl">
    <form action="{{ route('admin.glosarium.update', $glosarium->id) }}" method="POST">
        @csrf
        @method('PUT')
        <div class="p-6 space-y-6">
            ...
            <div>
                ...
                <input type="text" name="istilah" id="istilah" value="{{ old('istilah', $glosarium->istilah) }}" class="mt-1 focus:ring-bps-secondary focus:border-bps-secondary block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" required>
                @error('istilah')
            ...
            <div>
                ...
                <textarea name="definisi" id="definisi" rows="5" class="mt-1 focus:ring-bps-secondary focus:border-bps-secondary block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" required>{{ old('definisi', $glosarium->definisi) }}</textarea>
                @error('definisi')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
