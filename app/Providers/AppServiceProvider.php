    ...
    public function boot(): void
    {
        ...
        View::composer('layouts.app', function ($view) {
            if (Auth::check()) {
                ...
                $baseQuery = Pengumuman::where('status', 'Published')
                ...
                $activePeriode = PeriodePenilaian::where('status', 'voting')->first();
                if ($activePeriode && in_array($roleName, ['Pegawai'])) { // Or roles that should vote
                    $hasVoted = SurveyProgress::where('periode_id', $activePeriode->id)
                    ...
                    if (! $hasVoted && ! $isVotingPage) {
                        $votingReminder = new Pengumuman([
                            'id' => 'auto-voting-reminder-'.$activePeriode->id,
                            'judul' => 'Pengingat: Waktunya Voting!',
                            'konten' => "Periode voting \"{$activePeriode->nama}\" sedang berlangsung. Silakan berikan suara Anda sebelum ".Carbon::parse($activePeriode->tanggal_selesai_voting)->format('d M Y')." dengan mengklik menu 'Voting Kandidat Terbaik'. Suara Anda sangat berarti!",
                            'kategori' => 'Peringatan',
                            'prioritas' => 'High',
                            'is_sticky' => true,
                            'created_at' => Carbon::parse($activePeriode->tanggal_mulai_voting),
                        ]);
                ...
                $persiapanPeriode = PeriodePenilaian::where('status', 'penginputan')->first();
                if ($persiapanPeriode && in_array($roleName, ['Admin', 'Kepala Umum', 'Kepala_Umum'])) {
                    ...
                    $batasPersiapan = $persiapanPeriode->tanggal_selesai_persiapan
                        ? Carbon::parse($persiapanPeriode->tanggal_selesai_persiapan)->startOfDay()
                        : Carbon::parse($persiapanPeriode->tanggal_mulai)->startOfDay()->addDays(4);

                    // Jika H-2 atau sudah lewat dan data belum lengkap
                    if ($now->diffInDays($batasPersiapan, false) <= 2 && ! $persiapanPeriode->isDataLengkap()) {
                        $kelengkapanReminder = new Pengumuman([
                ...
                if ($activePeriode && in_array($roleName, ['Admin', 'Kepala Umum', 'Kepala_Umum'])) {
                    $now = Carbon::now()->startOfDay();
                    $batasVoting = Carbon::parse($activePeriode->tanggal_selesai_voting)->startOfDay();

                    // Jika H-1 atau sudah lewat (menuju akhir voting)
                    if ($now->diffInDays($batasVoting, false) <= 1) {
                        $totalPegawaiVoter = Pegawai::whereHas('role', function ($q) {
                            $q->whereIn('tipe', ['Pegawai', 'Kepala Umum', 'Kepala_Umum']);
                        })->count();

                        $sudahVotingCount = SurveyProgress::where('periode_id', $activePeriode->id)
                            ->distinct('user_id')
                            ->count('user_id');
                ...
                $globalSticky = $globalSticky->sortByDesc(function ($item) {
                    ...
                    if ($item->created_at instanceof Carbon || $item->created_at instanceof \DateTime) {
