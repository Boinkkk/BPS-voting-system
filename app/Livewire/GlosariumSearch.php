...
class GlosariumSearch extends Component
{
    ...
    public function render()
    {
        ...
        if (! empty(trim($this->search))) {
            ...
            $glosariums = $glosariums->filter(function ($item) use ($keyword) {
                return Str::contains(Str::lower($item->istilah), $keyword) ||
                       Str::contains(Str::lower($item->definisi), $keyword);
            })->values();
        }

        return view('livewire.glosarium-search', [
            'glosariums' => $glosariums,
        ]);
    }
}
